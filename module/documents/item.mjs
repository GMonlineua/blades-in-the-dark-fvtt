...
export default class BitdItem extends Item {
  ...
  async addLinkedItem(data) {
    ...
    if (!key) return ui.notifications.warn(game.i18n.format("BITD.Errors.Item.NotSupported", {item: item.name, type: localizeType}));

    const container = this.system[key];
    const idExist = container.some(existingItem => existingItem.id === item.id);
    const nameExist = container.some(existingItem => existingItem.name === item.name);

    if (idExist) return ui.notifications.error(game.i18n.localize("BITD.Errors.Item.ExistsId"));
    if (nameExist) ui.notifications.warn(game.i18n.localize("BITD.Errors.Item.ExistsName"));
  ...
  async addLinkedActor(data) {
    ...
    if (actor.type != "npc") ui.notifications.error(game.i18n.format("BITD.Errors.Actor.NotSupported", { type: localizeType, actor: actor.name }));
      ...
      for (const index in container) {
        if (container[index].uuid) {
          ...
          if (data) {
            ...
            const localizeType = game.i18n.localize("TYPES.Item." + container[index].type);
            ui.notifications.error(game.i18n.format("BITD.Errors.Item.NotExist", {type: localizeType, item: container[index].name}));
