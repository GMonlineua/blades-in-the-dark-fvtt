...
export default class BitdActor extends Actor {
    ...
    _onCreateDescendantDocuments(parent, collection, documents, data, options, userId) {
        ...
        if (game.user.id === userId) {
            for (const dataItem of data) {
                if (dataItem.type === "claim") claimMap(this, dataItem.type);
                if (dataItem.type === "prisonClaim") claimMap(this, dataItem.type);
    ...
    _onUpdateDescendantDocuments(parent, collection, documents, changes, options, userId) {
        ...
        if (game.user.id === userId) {
            for (const changeData of changes) {
                ...
                if (item.type === "claim") claimMap(this, item.type);
                if (item.type === "prisonClaim") claimMap(this, item.type);
            }
        }
    }
    ...
    async _preCreateContainer(container, forLoad) {
        ...
        if (container.type === "playbook") {
            ...
            // Load empty matrix
            const defaultClaims = Array.from({ length: 15 }, () => ({ ...CONFIG.BITD.claims.empty.actor }));
